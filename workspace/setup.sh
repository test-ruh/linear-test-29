#!/usr/bin/env bash
# =============================================================================
# One-command setup for Linear Todo Notifier
...
AGENT_NAME="Linear Todo Notifier"
AGENT_ID="linear-todo-notifier"
...
phase_env() {
  ...
  "LINEAR_API_KEY|Linear API key|Used to read your assigned Linear tickets and ticket details for reminders and the daily report.|Open https://linear.app in your browser and sign in.\nClick your workspace name, then open Settings.\nOpen the 'API' or developer access section.\nCreate a personal API key or copy an existing one.\nCopy the API key and paste it into the field above."
  "SLACK_BOT_TOKEN|Slack bot token|Used to send Slack direct messages for reminders and the daily report.|Open https://api.slack.com/apps in your browser and sign in.\nClick 'Create New App' or open the app you want to use.\nOpen 'OAuth & Permissions' in the left menu.\nInstall or reinstall the app to your workspace if needed.\nCopy the 'Bot User OAuth Token' value and paste it into the field above."
  "SLACK_USER_ID|Slack user ID|The Slack user ID that should receive the direct messages.|Open https://slack.com/app_redirect?channel=@me in your browser and sign in.\nOpen your own Slack profile in the workspace.\nClick the three-dot menu and choose 'Copy member ID' if available.\nIf you do not see it, ask a workspace admin to copy your member ID.\nCopy the Slack user ID and paste it into the field above."
  "SLACK_BOT_TOKEN|Slack DM bot token|Used by the agent to send Slack direct messages to you only.|Open https://api.slack.com/apps in your browser and sign in.\nClick 'Create New App' or open the app you want to use.\nOpen 'OAuth & Permissions' in the left menu.\nInstall or reinstall the app to your workspace if needed.\nCopy the 'Bot User OAuth Token' value and paste it into the field above."
  )

  # ── Optional vars (press Enter to skip) ──
  local optional_vars=(
  # No optional user vars
  )

  prompt_var() {
    local key="$1" label="$2" description="$3" how_to_get_steps="$4" is_required="$5"
    local current="${!key:-}"
    if [[ -n "${current}" ]]; then
      ok "  ${key} already set"
      return
    ...
    # Steps are joined with literal "\n"; %b interprets them as real newlines.
    if [[ -n "${how_to_get_steps}" ]]; then
      printf "  ${DIM}How to get:${RST}\n"
      printf "  ${DIM}  %b${RST}\n" "${how_to_get_steps}"
    fi
    ask "  ${key}:"
    local val
  ...
  for entry in "${required_vars[@]}"; do
    IFS='|' read -r key label description how_to_get_steps <<< "${entry}"
    prompt_var "${key}" "${label}" "${description}" "${how_to_get_steps}" "true"
  done

  for entry in "${optional_vars[@]}"; do
    IFS='|' read -r key label description how_to_get_steps <<< "${entry}"
    prompt_var "${key}" "${label}" "${description}" "${how_to_get_steps}" "false"
  done
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "  3. Launch via Docker with your own password:"
      warn "       docker run -d --name pg -e POSTGRES_PASSWORD='<choose-a-password>' -p 5432:5432 postgres:16"
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:<choose-a-password>@localhost:5432/linear-todo-notifier"
      exit 1
    fi
...
phase_finish() {
  ...
  # Register cron jobs from cron/jobs.json through the OpenClaw CLI.
  bash cron/register-cron.sh
  ...
  log "  openclaw cron list --all                # find job ids"
  log "  openclaw cron run <job-id>              # trigger manually"
